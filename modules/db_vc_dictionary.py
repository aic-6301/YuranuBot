...
def get_dictionary(server_id):
    ...
    """
    cursor.execute(
        f'CREATE TABLE IF NOT EXISTS "{server_id}" (text TEXT NOT NULL UNIQUE, reading TEXT NOT NULL, user INTEGER)'
    )
...
def save_dictionary(server_id: int, text, reading, user: int):
    ...
    try:
        ...
        cursor.execute(
            f'INSERT OR REPLACE INTO "{server_id}" (text, reading, user) VALUES(?, ?, ?)',
            (text, reading, user),
        )
        conn.commit()

        logging.debug(
            f"Server '{server_id}' Dictionary was updated ({text}: {reading})")
        return
...
def delete_dictionary(server_id: int, text):
    ...
    try:
        ...
        logging.debug(
            f"Server '{server_id}' Dictionary was updated (deleted: {text})")
        return
