    ...
    if after.channel is not None:
        ...
        if auto_channel == after.channel.id:
            if (not member.guild.voice_client
                    or member.guild.voice_client.channel != after.channel):
    ...
    if member.guild.voice_client and before.channel != after.channel:
        ...
        if before.channel != after.channel:
            for bot_client in bot.voice_clients:
                # 参加時に読み上げる
                if after.channel is not None:
                    _announce = get_server_setting(after.channel.guild.id,
                                                   "vc_user_announce")
                    mess = get_server_setting(after.channel.guild.id,
                                              "vc_join_message")  # ==が参加したのだ
                    ...
                    if after.channel.id == bot_client.channel.id:
                        if _announce == 2:
                            ...
                            if user_mess == "nan" and mess is not None:
                                await yomiage(f"{member.display_name}{mess}",
                                              member.guild)

                            elif user_mess is not None:
                                user_mess = user_mess.replace(
                ...
                # 退席時に読み上げる
                if before.channel is not None:
                    _announce = get_server_setting(before.channel.guild.id,
                                                   "vc_user_announce")
                    mess = get_server_setting(before.channel.guild.id,
                                              "vc_exit_message")  # ==が退席したのだ

                    if before.channel.id == bot_client.channel.id:
                        if _announce == 2:
                            user_mess = get_user_setting(
                                member.id, "disconn_msg")
    ...
    if member.guild.voice_client:
        if after.channel == member.guild.voice_client.channel:
            ...
            if before.self_video != after.self_video:
                if after.self_video:
                    await yomiage(
                        f"{member.display_name}がカメラ配信を開始しました。",
                        member.guild,
                    )
                else:
                    await yomiage(
                        f"{member.display_name}がカメラ配信を終了しました。",
                        member.guild,
                    )
            ...
            if before.self_stream != after.self_stream:
                if after.self_stream:
                    await yomiage(f"{member.display_name}が画面共有を開始しました。",
                                  member.guild)
                else:
                    await yomiage(f"{member.display_name}が画面共有を終了しました。",
                                  member.guild)
