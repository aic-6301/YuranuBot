...
class Discord_URL_Loader(commands.Cog):
    ...
    @app_commands.command(
        name="discord-url-load",
        description="DiscordのメッセージURLからメッセージを取得するのだ",
    )
    ...
    async def discord_url_load(self, interact: discord.Interaction, use: int):
        # DBに保存
        result = save_server_setting(interact.guild.id, "discord_url_load",
                                     use)
        ...
        if result == None:
            ...
            await interact.response.send_message(
                f"Discord URL Loaderを**「{use_str}」**に設定したのだ！")
            return
                ...
                if guild != None:
                    ...
                    if channel != None:
                        if channel.id == 1274365861410377749:
                            ...
                            tar_message = await channel.fetch_message(
                                int(message_id))
                        ...
                        if tar_message != None:
                            ...
                            if guild.icon == None or tar_message == None:
                                ...
                                file = discord.File(R"images\guest.png",
                                                    filename="guest.png")
                            ...
                            if guild.icon != None:
                                ...
                                embed.set_author(
                                    icon_url="attachment://guest.png",
                                    name=author_str)

                            # 送信元のユーザーアイコン
                            if tar_message.author.avatar != None:
                                embed.set_thumbnail(
                                    url=tar_message.author.avatar.url)
                            else:
                                embed.set_thumbnail(
                                    url="attachment://guest.png")
