...
from modules.db_vc_dictionary import (
    dictionary_load,
    delete_dictionary,
    save_dictionary,
    get_dictionary,
...
class yomiage_cmds(commands.Cog):
    ...
    @app_commands.command(
        name="vc-start",
        description="ユーザーが接続しているボイスチャットに接続するのだ",
    )
    async def vc_command(self, interact: discord.Interaction):
        try:
            ...
            if channel is None:
                ...
                channel = f"<#{channel}>"
            length_limit = get_server_setting(interact.guild_id,
                                              "length_limit")
            yomiage_speed = get_server_setting(interact.guild_id,
                                               "speak_speed")
            ...
            embed.add_field(name="読み上げるチャンネル", value=f"> {channel}")
            embed.add_field(name="読み上げ文字数の制限",
                            value=f"> {length_limit}",
                            inline=False)
            embed.add_field(name="読み上げスピード",
                            value=f"> {yomiage_speed}",
                            inline=False)
            embed.add_field(
                name="**VOICEVOXを使用しています!**",
                value=
                "**[VOICEVOX、音声キャラクターの利用規約](<https://voicevox.hiroshiba.jp/>)を閲覧のうえ、正しく使うのだ！**",
                inline=False,
            )
            embed.add_field(
            ...
            file = discord.File(R"images\boticon_zunda.png",
                                filename="boticon_zunda.png")
    ...
    async def check_yomi_settings(self, interact: discord.Interaction):
        ...
        if vc_channel is None:
            ...
            vc_channel = f"<#{vc_channel}>"

        spker_id = get_server_setting(interact.guild.id, "vc_speaker")
        spker_name = find_spker(id=spker_id)
        spker_name = spker_name[0]

        auto_conn_channel = get_server_setting(interact.guild.id,
                                               "auto_connect")
        if auto_conn_channel == 0 or auto_conn_channel is None:
            auto_conn_channel = "オフ"
        else:
            auto_conn_channel = f"<#{auto_conn_channel}>"
        ...
        soundtextMode_ = get_server_setting(interact.guild.id,
                                            "soundtext_mode")
        if soundtextMode_ == 2:
            soundtextMode = "有効"
        elif soundtextMode_ == 1:
            soundtextMode = "ゲームモード"
        elif soundtextMode_ == 0:
            soundtextMode = "無効"
        else:
            soundtextMode = "設定取得時にエラー発生"

        embed = discord.Embed(title="サーバーの読み上げ設定を表示するのだ！",
                              color=discord.Color.green())
        embed.add_field(name="読み上げるサーバー",
                        value=f"> {vc_channel}",
                        inline=False)
        embed.add_field(name="読み上げ話者", value=f"> {spker_name}", inline=False)
        embed.add_field(name="読み上げ速度",
                        value=f"> {vc_speak_speed}",
                        inline=False)
        embed.add_field(name="読み上げ文字制限",
                        value=f"> {length_limit}文字",
                        inline=False)
        embed.add_field(name="VCへの自動接続",
                        value=f"> {auto_conn_channel}",
                        inline=False)
        embed.add_field(name="サウンドテキスト機能", value=f"> {soundtextMode}")
        embed.set_footer(
            text=f"{self.bot.user.display_name} | Made by yurq.",
            icon_url=self.bot.user.avatar.url,
        )
    ...
    async def check_user_yomi_settings(self, interact: discord.Interaction):
        ...
        if user_speed == 0:
            user_speed = "サーバー設定を使用"
        connect_msg = get_user_setting(interact.user.id, "conn_msg")
        if connect_msg == "nan":
            connect_msg = "デフォルト設定"
        disconnect_msg = get_user_setting(interact.user.id, "disconn_msg")
        if disconnect_msg == "nan":
            disconnect_msg = "デフォルト設定"

        # Embedに設定内容を表示
        embed = discord.Embed(title="ユーザーの読み上げ設定を表示するのだ！",
                              color=discord.Color.green())
        embed.add_field(name="読み上げ話者", value=f"> {spk_name}", inline=False)
        embed.add_field(name="読み上げ速度", value=f"> {user_speed}")
        embed.add_field(name="接続メッセージ", value=f"> {connect_msg}", inline=False)
        embed.add_field(name="切断メッセージ",
    ...
    async def yomiage_channel(self, interact: discord.Interaction,
                              channel: discord.TextChannel):
        try:
            result = save_server_setting(interact.guild_id, "speak_channel",
                                         channel.id)
            if result is None:
                await interact.response.send_message(
                    f"☑**「<#{channel.id}>」**を読み上げるのだ！")
                return
    ...
    async def yomiage_channel(self, interact: discord.Interaction,
                              activate: int):
        try:
            result = save_server_setting(interact.guild_id, "vc_user_announce",
                                         activate)

            if result is None:
                if activate == 0:
                    await interact.response.send_message(
                        f"ユーザーの入退出読み上げを**「無効」**にしたのだ！")
                elif activate == 1:
                    await interact.response.send_message(
                        f"ユーザーの入退出読み上げを**「有効(ユーザー別無効)」**にしたのだ！")
                elif activate == 2:
                    await interact.response.send_message(
                        f"ユーザーの入退出読み上げを**「有効(ユーザー別有効)」**にしたのだ！")
                return
    ...
    async def vc_dictionary(self, interact: discord.Interaction, text: str,
                            reading: str):
        try:
            result = save_dictionary(interact.guild.id, text, reading,
                                     interact.user.id)
            if result is None:
                embed = discord.Embed(
                    title="正常に登録したのだ！",
                ...
                embed.add_field(name="登録した単語", value=text)
                embed.add_field(name="読み仮名", value=reading)
                embed.set_footer(
                    text=f"{self.bot.user.display_name} | Made by yurq.",
                    icon_url=self.bot.user.avatar.url,
                )
            ...
            if result:
                ...
                for i in range(len(result)):
                    ...
                    if (i + 1) % 10 == 0:  # Create a new embed every 10 words
                        ...
                        embed.set_footer(
                            text=
                            f"{self.bot.user.display_name} | Made by yurq.",
                            icon_url=self.bot.user.avatar.url,
                        )

                if (len(embed.fields) > 0
                    ):  # Add the last embed if there are remaining fields
                    embeds.append(embed)
    ...
    async def vc_dictionary(self, interact: discord.Interaction, text: str):
        try:
            ...
            if result is None:
                ...
                embed.add_field(name="削除した単語", value=text)
                embed.set_footer(
                    text=f"{self.bot.user.display_name} | Made by yurq.",
                    icon_url=self.bot.user.avatar.url,
                )
    ...
    async def soundboard_list(self, interact: discord.Interaction):
        try:
            ...
            if result:
                ...
                for i in range(len(result)):
                    embed.add_field(name=f"サウンドテキスト{i+1}",
                                    value=f"{result[i][0]}")
    ...
    async def soundtext_mode(self, interact: discord.Interaction, mode: int):
        try:
            result = save_server_setting(interact.guild_id, "soundtext_mode",
                                         mode)
            if result is None:
                mode_str: str = None
    ...
    async def yomiage_server_speaker(self, interact: discord.Interaction,
                                     id: int):
        try:
            if interact.user.guild_permissions.administrator:
                ...
                if result is None:
                    ...
                    await interact.response.send_message(
                        f"サーバー話者を**{spker_name[0]}**に変更したのだ！")
                    return
    ...
    @yomi.command(
        name="user-speaker",
        description="ユーザーの読み上げ話者を設定するのだ(どのサーバーでも同期されるのだ)",
    )
    @app_commands.rename(id="話者")
    @app_commands.choices(id=user_spk_choices)
    async def yomiage_user_speaker(self, interact: discord.Interaction,
                                   id: int):
        try:
            read_type = "vc_speaker"
            ...
            if result is None:
                if id == 1:
                    await interact.response.send_message(
                        f"ユーザー話者を**サーバー設定を使用**に変更したのだ！")
                else:
                    spker_name = find_spker(id=id)
                    await interact.response.send_message(
                        f"ユーザー話者を**{spker_name[0]}**に変更したのだ！")
                    return
    ...
    @yomi.command(
        name="join-message",
        description="参加時の読み上げ内容を変更するのだ<<必ず最初にユーザー名が来るのだ>>",
    )
    async def change_vc_join_message(self, interact: discord.Interaction,
                                     text: str):
        try:
            res = save_server_setting(interact.guild_id, "vc_join_message",
                                      text)
            if res is None:
    ...
    @yomi.command(
        name="exit-message",
        description="退出時の読み上げ内容を変更するのだ<<必ず最初にユーザー名が来るのだ>>",
    )
    async def change_vc_exit_message(self, interact: discord.Interaction,
                                     text: str):
        try:
            res = save_server_setting(interact.guild.id, "vc_exit_message",
                                      text)
            if res is None:
    ...
    @yomi.command(
        ...
        description="[ユーザー別]参加時の読み上げを設定するのだ！",
    )
    @app_commands.describe(text="<user> : ユーザー名")
    async def change_user_join_message(self, interact: discord.Interaction,
                                       text: str):
        try:
            res = save_user_setting(interact.user.id, "conn_msg", text)
            if res is None:
                await interact.response.send_message(
                    "**ユーザー別 参加時の読み上げ内容を変更したのだ！**")
                return
    ...
    @yomi.command(
        name="user-exit-message",
        description="[ユーザー別]退席時の読み上げを設定するのだ！",
    )
    @app_commands.describe(text="<user>: ユーザー名")
    async def change_user_exit_message(self, interact: discord.Interaction,
                                       text: str):
        try:
            res = save_user_setting(interact.user.id, "disconn_msg", text)
            if res is None:
                await interact.response.send_message(
                    "**ユーザー別 退出時の読み上げ内容を変更したのだ！**")
                return
    ...
    @yomi.command(
        name="auto-channel",
        description="設定したVCに自動接続するのだ(現在入っているVCが対象なのだ)",
    )
        ...
        try:
            if bool is True:
                ...
                if (interact.user.voice
                        is not None):  # 設定するユーザーがチャンネルに入っていることを確認するのだ
                    res = save_server_setting(interact.guild_id,
                                              "auto_connect", vc_id)

                else:  # ユーザーがボイスチャットに入っていない場合
                    await interact.response.send_message(
                        "自動接続したいチャンネルに入ってから実行するのだ！")
                    return
            else:
    ...
    async def vc_disconnect_command(self, interact: discord.Interaction):
        try:
            if interact.guild.voice_client is None:
                ...
                await interact.response.send_message(
                    "ボイスチャンネルに接続していないのだ...入ってから実行するのだ")
                return
            elif interact.user.voice.channel != interact.guild.voice_client.channel:
                await interact.response.send_message(
                    "入ってるボイスチャンネルと違うチャンネルなのだ...実行してるチャンネルでやるのだ")
                return
