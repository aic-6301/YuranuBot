...
class Computer(commands.Cog):
    ...
    if os.getenv("USE_RPC") == "True":
        ...
        async def rpc_loop(self):
            ...
            if self.rpc_mode == 1:
                ...
                user_count = sum(
                    len(guild.members) for guild in self.bot.guilds)
                ...
                activity_message = (
                    f"起動時間: {int(days)}日 {int(hours)}時間 {int(minutes)}分")
                status = Status.online

            elif self.rpc_mode == 3:
                pc = await pc_status()

                if (pc.cpu_load >= 90 or pc.gpu_load >= 90
                        or pc.gpu_mem_percent >= 90 or pc.ram_percent >= 90):
                    activity_message = "⚠リソース使用率: 高"
                    status = Status.dnd
                elif (pc.cpu_load >= 70 or pc.gpu_load >= 70
                      or pc.gpu_mem_percent >= 70 or pc.ram_percent >= 70):
    ...
    # PCの状態
    @app_commands.command(name="status", description="Botを稼働しているPCの状態を表示するのだ")
    async def status(self, interact: discord.Interaction):
        ...
        embed = discord.Embed(title="サーバーの稼働状況なのだ！",
